...
export default function Activar() {
  ...
  const handleSearch = async (e) => {
    ...
    const title=joinData? 'Éxito': 'Info';
  ...
  const handleSavePermissions = async () => {
    if (!loteFound) return;
    setIsSaving(true);
    const result = await putLote(dataSetForm);
    if (!result.success) {
            ...
            <div className="bg-white shadow overflow-hidden sm:rounded-lg">
              <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
                <h3 className="text-lg leading-6 font-medium text-gray-900">
                  ...
                  {loteFound ? 'Activar o Desactivar Lote' : 'Buscar Lote'}
                </h3>
                <p className="mt-1 max-w-2xl text-sm text-gray-500">
                  {loteFound 
                    ? 'Revise y active o desactive el lote.'
                    : 'Ingrese el código de lote para buscar un lote.'}
                </p>
              ...
              {loteFound && (
                <div className="mt-8 bg-white shadow overflow-hidden sm:rounded-lg">
                  <div className="px-4 py-5 sm:px-6">
                    <h3 className="text-lg leading-6 font-medium text-gray-900">
                      Activar o desactivar Lote
                    </h3>
                    <p className="mt-1 max-w-2xl text-sm text-gray-500">
                      Insumo: {dataSetForm?.nombre}
                    </p>
                    <p className="mt-1 max-w-2xl text-sm text-gray-500">
                      Lote: {dataSetForm?.numeroLote}
                    </p>
                    <p className="mt-1 max-w-2xl text-sm text-gray-500">
                      Cantidad: {dataSetForm?.cantidad}
                    </p>
                  </div>
